...
from app.models.families import FamilyCreate
...
def test_create_family(db: Session):
    ...
    family = service.create(FamilyCreate(name=name))
...
def test_get_family(db: Session):
    ...
    fetched_family = service.get_by_id(family.id)

    assert fetched_family == family
